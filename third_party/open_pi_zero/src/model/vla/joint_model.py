...
def forward_mixture_attn(
    ...
    if use_flash_attn:
        dropout_p = (
            attention_dropout if mixtures[active_mixture_names[0]].training else 0.0
        )
        ...
        ) / math.sqrt(mixtures[active_mixture_names[0]].head_dim)
...
class JointModel(nn.Module):
    def __init__(self, config):
        ...
        self.kv_quant_config = config.get("kv_quantization")
    ...
    def build_mixture_caches(self):
        return {
            name: KVCache(self.kv_quant_config) for name in self.cache_names
        }
