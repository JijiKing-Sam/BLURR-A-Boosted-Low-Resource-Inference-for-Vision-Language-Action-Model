...
class PiZero(nn.Module, NoSyncBase):
    ...
    def __init__(self, cfg, use_ddp: bool = False):
        ...
        self.action_quant_config = cfg.get("action_quantization")
        self._action_quant_enabled = False
    ...
    def enable_action_quantization(self):
        if self._action_quant_enabled:
            return
        cfg = self.action_quant_config or {}
        mode = str(cfg.get("mode", "")).lower()
        if mode in {"", "none"}:
            return
        mixtures = getattr(self.joint_model, "mixtures", None)
        action_mixture = None
        if isinstance(mixtures, nn.ModuleDict):
            if "action" in mixtures:
                action_mixture = mixtures["action"]
        elif isinstance(mixtures, dict):
            action_mixture = mixtures.get("action")

        if mode in {"int8", "int8_cached"}:
            clip = cfg.get("activation_clip")
            cache_fp = cfg.get("cache_fp_weight", False)
            fp_dtype = cfg.get("fp_dtype", "bfloat16")
            quant_cfg = Int8QuantConfig(
                activation_clip=clip,
                cache_fp_weight=cache_fp,
                fp_dtype=fp_dtype,
            )
            if action_mixture is not None:
                quantize_module_int8(action_mixture, quant_cfg)
            quantize_module_int8(self.action_encoder, quant_cfg)
            quantize_module_int8(self.action_decoder, quant_cfg)
            quantize_module_int8(self.proprio_encoder, quant_cfg)
        elif mode == "bnb_int8":
            threshold = float(cfg.get("threshold", 0.0) or 0.0)
            bias_dtype = getattr(torch, cfg.get("bias_dtype", "float16"), torch.float16)
            if action_mixture is not None:
                quantize_module_bnb_int8(
                    action_mixture, threshold=threshold, bias_dtype=bias_dtype
                )
            quantize_module_bnb_int8(
                self.action_encoder, threshold=threshold, bias_dtype=bias_dtype
            )
            quantize_module_bnb_int8(
                self.action_decoder, threshold=threshold, bias_dtype=bias_dtype
            )
            quantize_module_bnb_int8(
                self.proprio_encoder, threshold=threshold, bias_dtype=bias_dtype
            )
        else:
            return
        self._action_quant_enabled = True
